...
class Routes  {
    ...
    buildChart(data = currentData, cb) {
    ...
    pointDistance = (point1, point2) => {
        ...
        let dist = Math.pow((Math.pow(px, 2) + Math.pow(py, 2)), 0.5);
        return dist;
    }
    ...
    processPoints = (dataset, cb) => {
        ...
        this.sortedRoute.push(dataset[0]);
        for (let i = 0; i < dataset.length; i++) {
            this.visited.push(this.queue[0]);
            nextPoint = this.findShortest(this.queue[0], dataset);
            this.queue = [];
            if (nextPoint) {
                this.sortedRoute.push(nextPoint);
                this.queue.push(nextPoint);
            }
        }
        // Take the first stop out of the visited list
        this.visited.shift();
        // And add it back in to the route at the end
        this.sortedRoute.push(dataset[0]);
        return cb();
    }
